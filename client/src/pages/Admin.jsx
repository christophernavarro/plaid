...
export default function Admin() {
  ...
  async function sendReminder() {
    if (!selected) return;
    const res = await api(`/api/admin/usuarios/${selected.id}/notificar`, {
      method: 'POST',
      body: JSON.stringify({ tipo: 'conectar_banco' }),
    });
    if (res.ok) {
    ...
    return (
      <div className="max-w-[940px] mx-auto px-5 py-8 pb-16">
        <div className="flex justify-between items-center">
          ...
          <button onClick={logout} className="flex items-center gap-1.5 px-4 py-2.5 border border-brand-200 bg-brand-50 rounded-[10px] text-sm font-medium text-brand-600 hover:bg-brand-100 hover:border-brand-300 transition-all">
        ...
        <div className="flex items-center gap-2">
          {datos && !datos.conectado && (
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2.5 rounded-[10px] text-sm font-semibold transition-all ${
                reminderSent
                  ? 'bg-green-50 text-pos border border-green-200'
                  : 'bg-gradient-to-b from-brand-500 to-brand-600 text-white shadow-lg shadow-brand-600/25 hover:brightness-110 hover:-translate-y-0.5'
              }`}
            >
              {reminderSent ? (
                <>
